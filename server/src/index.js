...
async function createRoom(env, request) {
  ...
  for (let attempt = 0; attempt < MAX_CREATE_ATTEMPTS; attempt++) {
    ...
    if (response.status !== 409) {
      ...
      return textResponse(
        errorText || "Could not create room.",
        500,
        request
      );
    }
  ...
  return json(
    {
      error:
        "Could not find an unused four-digit room code. Try again."
    },
    {
      status: 503
    },
...
export default {
  async fetch(request, env) {
    ...
    /*
     * Always answer CORS preflight requests before routing.
     */
     ...
     *
     * The Durable Object returns an internal response, so this
     * Worker explicitly re-applies the browser-facing CORS headers.
     */
    const roomMatch =
      url.pathname.match(/^\/api\/rooms\/([0-9A-Z]{4})$/i);
    ...
    if (roomMatch && request.method === "GET") {
      ...
      try {
        const stub = env.GAME_ROOM.get(
          roomId(env, code)
        );

        const response = await stub.fetch(
          "https://room.internal/info"
        );
        ...
        /*
         * Do not allow stale room information to be cached by
    ...
    const websocketMatch =
      url.pathname.match(/^\/ws\/([0-9A-Z]{4})$/i);
...
export class GameRoom extends DurableObject {
  constructor(ctx, env) {
    ...
    /*
     * Restore the lightweight connection index after
  ...
  async handleIdentify(
    ...
    if (
      typeof data.profileId ===
      "string"
    ) {
      participant.profileId =
        data.profileId.slice(
          0,
          100
        );
    }
